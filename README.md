...
- Clone this repo:
	```bash
	git clone https://github.com/lperezperez/font-patcher-helper.git
	cd font-patcher-helper
	```
- Run the script which will install (and or upgrade) all dependencies needed to run the font patcher:
	```bash
...
- If you want to download the latest [Nerd fonts](https://github.com/ryanoasis/nerd-fonts/tree/master/patched-fonts/), run the script:
	```bash
	./download-patched-fonts.py [font family name]...
	```
	>Fonts are downloaded into the `./patched-fonts` folder.
- If you want to rename fonts (Its `font` naming table) based on the [OpenType specifications](https://docs.microsoft.com/typography/opentype/spec/name#name-ids), run the script:
	```bash
	./rename-fonts.py [path]...
	```
	>The renamed fonts are generated into the `./renamed-fonts` folder.
- Once the script `./font-patcher-helper.py` is executed, if you want to test the current font, run the script:
	```bash
	./test-fonts.sh
	```
## Changelog
