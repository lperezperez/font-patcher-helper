...
- Install fontforge:
	...
	```
- Install `fontforge`, `configparser` and `foontools` python dependencies:
	```bash
	sudo apt-get install python3-fontforge python-configparser python3-fonttools
	```
- Clone this repo
	```bash
	...
	```
### Remarks
Could be necessary to replace the [Shebang](https://en.wikipedia.org/wiki/Shebang_(Unix)) tag of [`font-patcher`](https://github.com/ryanoasis/nerd-fonts/tree/master/font-patcher) script
- From
	```bash
	#!/usr/bin/env python
	```
